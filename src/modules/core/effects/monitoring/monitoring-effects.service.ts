...
export class MonitoringEffects {
  ...
  @Effect() addMetric$: Observable<Action> = this.actions$.pipe(
    ...
    switchMap(aggregation => {
      return this.metricService.addMetricSettings(aggregation).pipe(
        map(response => {
          this.mdlSnackbarService.showSnackbar({
            message: 'Metric was successfully added',
            timeout: 5000,
          });
          return new HydroActions.AddMetricSuccessAction(
  ...
  @Effect() getMetrics$: Observable<Action> = this.actions$.pipe(
    ...
    switchMap(modelVersionId => {
      return this.metricService.getMetricSettings(modelVersionId).pipe(
        map(response => {
          const metricSpecifications = response.map(
            _ => new MetricSpecification(_)
          );
  ...
  @Effect() deleteMetric$: Observable<Action> = this.actions$.pipe(
    ...
    switchMap(id => {
      return this.metricService.deleteMetricSettings(id).pipe(
        map(_ => {
