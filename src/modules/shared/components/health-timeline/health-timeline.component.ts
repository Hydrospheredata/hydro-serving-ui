...
export class HealthTimelineComponent implements OnInit, OnDestroy {
  ...
  private getFullAggregation(): Observable<IMonitoringAggregationList> {
    const requests = this.metricSpecs.map(mS =>
  ...
  private updateBrush() {
    ...
    d3.select(this.brush.nativeElement)
  ...
  private elementsPositioning() {
    const rowCount = Object.keys(this.currentLogData).length;
    d3.select(this.dataContainer.nativeElement).attr(
      'transform',
      `translate(0, ${this.MARGIN.top})`
    );

    this.xAxisTransform = `translate(0, ${this.MARGIN.top +
      rowCount * this.LINE_HEIGHT})`;
    this.mimimapTransform = `translate(0, ${this.MARGIN.top +
      rowCount * this.LINE_HEIGHT +
      40})`;
  }
