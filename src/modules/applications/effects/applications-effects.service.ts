...
export class ApplicationsEffects {
  @Effect() getApplications$: Observable<Action> = this.actions$.pipe(
    ...
    switchMap(() => {
      return this.applicationsService.getApplications().pipe(
        ...
        catchError(error =>
          observableOf(new HydroActions.GetApplicationsFailAction(error))
        )
      );
    })
  );

  @Effect() addApplication$: Observable<Action> = this.actions$.pipe(
    ofType(HydroActions.ApplicationActionTypes.Add),
    map((action: HydroActions.AddApplicationAction) => action.application),
    switchMap(application => {
      return this.applicationsService.addApplication(application).pipe(
        map(response => {
  ...
  @Effect() updateApplication$: Observable<Action> = this.actions$.pipe(
    ofType(HydroActions.ApplicationActionTypes.Update),
    map((action: HydroActions.UpdateApplicationAction) => action.application),
    switchMap(application => {
      return this.applicationsService.updateApplication(application).pipe(
        map(response => {
          this.mdlSnackbarService.showSnackbar({
        ...
        catchError(error => {
          ...
          return observableOf(
            new HydroActions.UpdateApplicationFailAction(error)
          );
        })
      );
    })
  ...
  @Effect() deleteApplication$: Observable<Action> = this.actions$.pipe(
    ofType(HydroActions.ApplicationActionTypes.Delete),
    map((action: HydroActions.DeleteApplicationAction) => action.application),
    switchMap(({ name }) => {
      return this.applicationsService.deleteApplication(name).pipe(
        map(() => {
        ...
        catchError(error => {
          ...
          return observableOf(
            new HydroActions.DeleteApplicationFailAction(error)
          );
        })
      );
    })
  ...
  @Effect() generateInputs$: Observable<Action> = this.actions$.pipe(
  ...
  @Effect() setInputs$: Observable<Action> = this.actions$.pipe(
    ...
    switchMap(([action, { name }]) => {
      return observableOf(
        new HydroActions.SetInputSuccessAction({ name, input: action })
      );
    })
  );

  @Effect() testApplication$: Observable<Action> = this.actions$.pipe(
      ...
      return this.applicationsService
        ...
        .pipe(
          ...
          catchError(error => {
            ...
            return observableOf(
              new HydroActions.TestApplicationFailAction(payload)
            );
          })
        );
    })
