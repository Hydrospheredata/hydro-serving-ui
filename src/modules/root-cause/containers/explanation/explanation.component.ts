...
export class ExplanationComponent implements OnChanges, OnDestroy {
  ...
  private inputChanged: BehaviorSubject<any> = new BehaviorSubject<any>(undefined);

  constructor(private readonly service: ExplanationService, private readonly dialogService: DialogService) {
    this.getExplanationCommand.asObservable().pipe(
      switchMap(() => {
        return this.service.createExplanation(this.modelVersionId, this.requestId)
      }),
      takeUntil(this.destroy.asObservable())
    ).subscribe()


    this.explanation$ = merge(
      this.inputChanged.asObservable()
    ).pipe(
      switchMap(() => this.service.getExplanation(this.requestId, this.modelVersionId).pipe(
        switchMap((explanation) => {
          if (explanation.state === 'SUCCESS' ||
  ...
  ngOnChanges(changes: SimpleChanges): void {
    if (changes.requestId && changes.requestId.currentValue || changes.modelVersionId && changes.modelVersionId.currentValue) {
  ...
  private pullingExplanation() {
    return timer(1000, 10000).pipe(
      switchMap(() => this.service.getExplanation(this.requestId, this.modelVersionId)),
